...
class ServerTest(unittest.TestCase):
    ...
    def test_image_render_marker_is_stable_across_requests(self):
        # transformers caches compiled Jinja templates keyed by the template
        # *source string* (utils.chat_template_utils._compile_jinja_template).
        # A marker that changed per request made that cache string differ on
        # every image request, forcing a full Jinja recompile each time. The
        # marker substituted into the template source must stay byte-for-byte
        # identical across separate requests so that cache keeps hitting.
